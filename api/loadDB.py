...
def get_places_for_type(typ, subType=None):
    ...
    if subType:
        apiCall += '.' + subType
    apiCall += '&filter=rect:' + str(london_longitude1) + ',' + str(
        london_latitude1) + ',' + str(london_longitude2) + ',' + str(
        london_latitude2)
    ...
    if response.status_code == 200:
        for feature in response.json()['features']:
            attraction = {'name': feature['properties']['name'], 'type': typ,
...
def load_db_with_data():
    ...
    places = [get_places_for_type('tourism', 'attraction'),
    ...
    for place_type in places:
        for place in place_type:
            if place['type'] == 'tourism':
                params = (place['name'], place['type'], place['subtype'],
                          place['post_code'], place['latitude'],
                          place['longitude'])
            else:
                params = (place['name'], place['subtype'], None,
                          place['post_code'], place['latitude'],
                          place['longitude'])
            try:
